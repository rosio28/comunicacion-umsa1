...
// ============================================================
// SESIONES DE EVENTOS — nuevas funcionalidades
// ============================================================
class EventoSesionesController {
    ...
    public function store(int $eventoId): void {
        ...
        $numSesion = isset($d['numero_sesion'])&&$d['numero_sesion']!==''?(int)$d['numero_sesion']:null;
        if(!$numSesion){$q=$db->prepare("SELECT COALESCE(MAX(numero_sesion),0)+1 FROM evento_sesiones WHERE evento_id=?");$q->execute([$eventoId]);$numSesion=(int)$q->fetchColumn();}
        ...
        $stmt->execute([$eventoId,$numSesion,$titulo,$d['fecha']??null,$d['descripcion']??null,$d['contenido_visto']??null,$d['material_url']??null,$d['grabacion_url']??null,$d['enlace_virtual']??null,isset($d['publicado'])?($d['publicado']?'true':'false'):'true']);
        Response::success($stmt->fetch(),'Sesión registrada',201);
    }
    public function update(int $sesionId): void {
        $payload = Auth::requireAuth(); Auth::requireRole($payload,['admin','superadmin']);
        $d=$db_=null; $d=json_decode(file_get_contents('php://input'),true)??[]; $db=Database::getConnection();
        $fields=[];
        if(!empty($d['titulo']))         $fields['titulo']=$d['titulo'];
...
// ============================================================
// CONVENIOS INSTITUCIONALES — nuevas funcionalidades
// ============================================================
class ConveniosController {
    private function norm(array $c): array {
        if(!empty($c['fecha_vencimiento'])){
            $hoy=new DateTime(); $vcto=new DateTime($c['fecha_vencimiento']); $diff=$hoy->diff($vcto);
    ...
    public function index(): void {
        $db=$_db=null; $db=Database::getConnection(); $all=!empty($_GET['all']);
        $cond=$all?[]:['publicado=true']; $params=[];
        ...
        $stmt->execute($params); Response::success(array_map([$this,'norm'],$stmt->fetchAll()));
    }
    public function store(): void {
        $payload=Auth::requireAuth(); Auth::requireRole($payload,['admin','superadmin']);
        $isP=!empty($_POST); $d=$isP?$_POST:(json_decode(file_get_contents('php://input'),true)??[]);
        $nombre=trim($d['nombre_institucion']??''); if(!$nombre) Response::error('Nombre de la institución requerido');
